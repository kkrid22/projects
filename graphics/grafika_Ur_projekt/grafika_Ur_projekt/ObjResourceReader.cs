...
namespace grafika_Ur_projekt
{
    internal class ObjResourceReader
    {
        ...
        public static unsafe GlObject CreateObjectWithColor(GL Gl, float[] faceColor, string objectPath, string texPath)
        {
            ...
            ReadObjDataForObject(out objVertices, out objFaces, out objNormals, out objTexCoords, objectPath);
            ...
            return CreateOpenGlObject(Gl, vao, glVertices, glColors, glIndices, texPath);
        }

        private static unsafe GlObject CreateOpenGlObject(GL Gl, uint vao, List<float> glVertices, List<float> glColors, List<uint> glIndices, string path)
        {
            uint offsetPos = 0;
            ...
            var skyboxImageResult = ReadTextureImage(path);
        ...
        private static unsafe void ReadObjDataForObject(out List<float[]> objVertices, out List<faceData[]> objFaces, out List<float[]> objNormals, out List<float[]> objTexCoords, string path)
        {
            ...
            using (Stream objStream = typeof(ObjResourceReader).Assembly.GetManifestResourceStream("grafika_Ur_projekt.Resources." + path))
